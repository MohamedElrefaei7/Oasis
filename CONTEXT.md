...
tests/                   39 test modules, 1001 tests
...
- **`vector.py`** — `VectorIndex` over LanceDB. `vector` column is `pa.list_(pa.float32(), dimension)` (fixed-size list, required for ANN). `upsert_chunks` → `merge_insert("chunk_id")`; `search(vec, limit, where)` cosine, maps `_distance` → `score`; `delete_by_doc_id`, `count`, `doc_ids_with_vectors()` (one bulk doc_id projection — the backfill's existence check, computed once per run, never per doc). Also **`make_chunk_id(path, index)` / `is_name_chunk(chunk_id)`** (2026-08-02) — the one definition of the primary-key format. The pipeline composes it and retrieval takes it apart to keep name chunks out of the reranker's passage; nothing in the schema makes those two agree, so they share a function.
...
- **`reranker.py`** — `CrossEncoderReranker` (`cross-encoder/ms-marco-MiniLM-L-6-v2`, shared `_MODEL_CACHE`). `rerank(query, results, *, top_n)` scores `(query, _passage(r))` pairs, replaces `score` via `dataclasses.replace`, sorts desc. `_clean` strips `\x02`/`\x03` before scoring. **`_passage()` builds the judged passage: the humanized filename, then the best *content* chunk (falling back to the snippet)** — both parts 2026-08-02. The **name** stops this stage *undoing* the retrieval it reranks: a document found because its filename matched arrives with a content snippet that by construction contains none of the query terms, so a name-blind model buries what the two arms just surfaced (+0.011 ndcg / +0.029 mrr). Leading with a short label is also in-distribution — MS MARCO cross-encoders train on title-first passages. The **body** is the bigger half: an FTS snippet is a keyword-centred fragment and a chunk is prose, worth **+0.070 ndcg@10**, while a *longer* snippet is worth nothing. See Evaluation § 🟢 The reranker was starved.
...
- **`manager.py`** — `ensure_ollama(model)` → `OllamaProvider | None`, silently. `_server_running` (urlopen, 1s), `_model_available` (`ollama list`), `_start_server` (`shutil.which` → spawn `ollama serve` → poll to `_STARTUP_TIMEOUT=5.0`). **Read that third step again: this function is not a probe, it is a probe *and a daemon launcher*.** Everything that calls it is committing to possibly starting a background LLM server on the user's machine, which is why the API stopped calling it at startup (2026-08-02).
...
- **`raw` defaults to `True` — parsing is opt-in.** This deliberately diverges from the CLI (which parses unless `--raw`): the eval measured NL parsing as −0.108 ndcg@10 / −0.135 recall@10 on hybrid+CE, so parsing-off is the best-measured path and the endpoint's default. `raw=false` goes through **`state.get_llm()`**, which probes once on first use and caches (never per request — that's an `ollama list` subprocess per query — and never at startup, see below); `None` provider or any parse exception falls back to `ParsedQuery(semantic_query=q)` with `llm_parsed=false`. Search never fails because the LLM is absent.
...
> 3. **The cross-encoder briefly flipped to net-negative — ~~and that is the next thing to settle~~ SETTLED, same day, see below.** With the name chunk present but the reranker still judging FTS snippets, raw fusion scored **0.6531** and reranking pulled it **down to 0.6280**. The cause was the passage, not the model.

> ### 🟢 The reranker was starved, not wrong: **0.6280 → 0.6981** (2026-08-02)
>
> Chasing finding 3 above produced the single largest configuration win in the project. Hypothesis from Up Next: the cross-encoder judges relevance from a 20-token FTS snippet, and a document surfaced by its *filename* arrives carrying a keyword snippet about something else — so it is starved. Half right; the starvation is real and it is **not about length**.
>
> | passage given to the cross-encoder | ndcg@10 | recall@10 | mrr |
> |---|---|---|---|
> | no reranker at all (raw RRF fusion) | 0.6531 | 0.7963 | 0.6317 |
> | FTS snippet, 20 tokens (the old default) | 0.6280 | 0.7442 | 0.6331 |
> | FTS snippet, 48 tokens | 0.6161 | 0.7442 | 0.6140 |
> | FTS snippet, 96 tokens | 0.6277 | 0.7442 | 0.6317 |
> | FTS snippet, 200 tokens | 0.6277 | 0.7442 | 0.6317 |
> | best-matching vector chunk | 0.6264 | 0.7442 | 0.6318 |
> | **best-matching _content_ chunk** (shipping) | **0.6981** | **0.7713** | **0.7066** |
>
> **Two counter-intuitive results, and the second is the one to remember:**
>
> 1. **A longer snippet is worth nothing.** 20 → 200 tokens moves the metric by less than noise, and 48 is *worse* than 20. An FTS snippet is a keyword-centred fragment; ten times more fragment is still fragment. The model lacked coherent prose, not characters. Anyone who reaches for `SNIPPET_TOKENS` to improve ranking should read this row first.
> 2. **"Use the best-matching chunk" is a trap once filenames are indexed** — 0.6264, indistinguishable from the snippet. The reason is obvious in hindsight and invisible in advance: for a filename query *the closest chunk is the name chunk*, so the model is handed back the same three words it already matched. Excluding name chunks from the passage — while keeping them in the ranking, which is the entire point of having them — is the whole difference between 0.6264 and 0.6981. This is why `hybrid_search` tracks `best_content` alongside `best_vec`, and why `is_name_chunk` exists.
>
> **Display and reranking are now allowed to differ**, which they always should have been: a person wants the highlighted match, a cross-encoder wants a paragraph. `HybridResult.snippet` is unchanged and still what the UI renders; `rerank_text` is new and never displayed. Costs no extra query — the chunk is already in the candidate set the vector arm returned.
>
> Reranking is worth having again: **+0.045 over raw fusion**, roughly its original margin, and hybrid+CE is back on top of the matrix. The served path was re-verified byte-identical to the direct harness afterwards (`verify_served.py`, 0/83 ranking mismatches).
>
> **Left on the table, deliberately:** feeding the doc's *longest* chunk rather than its best-matching content chunk measured **0.7229** in the scratch experiment. Capturing that needs a `doc_id → chunk text` lookup against LanceDB per search (the candidate set doesn't contain it), i.e. a real query and real latency on the search path, for +0.025. Not taken without a latency budget to weigh it against — see Up Next.
>
...
## Tests — 1001, all passing

Run with `pixi run -e dev pytest` (fast: 998 + 3 `slow` deselected) or `pixi run -e dev pytest -m ''` (all 1001). Slow tests load real models on CPU. (952 until 2026-07-29, when the subtraction pass deleted the 13 tests covering the dead `fts_snippet`/`get_snippet` path along with the functions themselves; 939 until 2026-08-02, when filename indexing added 49 and the follow-up pass added 13.)

New 2026-08-02 (second pass): `test_retriever.py` +5 — `rerank_text` is the best **content** chunk, is `None` when only the name chunk matched or the hit was keyword-only, the name chunk still *ranks* even though it is excluded from the passage, and the display snippet is unchanged by any of it. `test_reranker.py` +3 — prose preferred over the snippet, the fallback, and markers stripped from it. `test_vector.py` +4 — the `chunk_id` round trip, including a path literally ending in `-1` not reading as a name chunk. `test_api_search.py` +1 — **the Ollama probe does not run at startup**, runs once on the first `raw=false` request, and is not re-run after a negative result.
...
| `test_retriever.py` | 72 | `test_cli_edges.py` | 23 | `test_human_size.py` | 16 |
| `test_vector.py` | 62 | `test_extractor_edges.py` | 21 | `test_api_skeleton.py` | 15 |
...
| `test_reranker.py` | 42 | `test_api_index.py` | 19 | `test_device.py` | 11 |
| `test_chunker.py` | 40 | `test_api_search.py` | 19 | `test_api_open.py` | 10 |
| `test_parser.py` | 37 | `test_walker_edges.py` | 19 | `test_models.py` | 10 |
...
| Model lifecycle | Config, embedder, cross-encoder and `VectorIndex` initialized **once** in app state and warmed with a throwaway inference; the LLM provider is probed lazily instead (see below). Loading runs on a **background thread**, not inline in lifespan — Uvicorn doesn't accept connections until lifespan startup returns, so a sync load makes `/api/health` connection-refused for its whole duration. The LLM provider is cached (incl. `None`) but resolved on **first use, not at startup** — `ensure_ollama()` *starts* `ollama serve`, so probing eagerly spun one up for a client that hardcodes `raw=true`. Per-search would be an `ollama list` subprocess spawn per query. |
...
- ~~**🔴 Settle whether the cross-encoder still earns its place.**~~ **Settled 2026-08-02, same day it was raised** — it was starved, not wrong, and feeding it the best *content* chunk instead of a 20-token FTS snippet took hybrid+CE from 0.6280 to **0.6981**, back above raw fusion. Full table and the two counter-intuitive results under Evaluation § 🟢 The reranker was starved. The one follow-on left: the scratch experiment's **longest**-chunk variant measured **0.7229**, another +0.025, but capturing it needs a per-search `doc_id → chunk text` lookup against LanceDB that the candidate set can't supply. **That is a latency question, not a quality one** — it should be taken only once there is a p95 budget to weigh it against, which there still isn't (see the Latency note in the README). Do not add a query to the search path on a metric alone.
...
**The one loose end it created — the cross-encoder flipping net-negative — was closed the same day**, in the follow-up pass below.

### Recently done (2026-08-02, second pass) — tying off the threads

Four items that had been flagged as "your call" or "next to settle", plus the retrieval win that came out of chasing one of them. **1001 tests** (939 → 988 → 1001), `ruff check .` clean.

**🟢 The reranker was starved, not wrong — 0.6280 → 0.6981.** The headline. Full table and the two counter-intuitive results under Evaluation. Short version: a longer FTS snippet is worth nothing, the *best-matching* chunk is a trap once name chunks exist (it hands the model back the words it already matched), and the best *content* chunk is worth +0.070. `HybridResult.rerank_text` now carries what the model judges, separate from the `snippet` a human reads. `verify_served.py` re-run afterwards: 0/83 ranking mismatches.

**🔴 The server no longer starts an Ollama server nobody asked for.** `ensure_ollama()` is not a probe — step three of it is `shutil.which("ollama")` → **spawn `ollama serve`** → poll for 5 s. `api/app.py` called that at every server start, so every launch of a menu-bar app that hardcodes `raw=true` and *cannot reach the LLM* started a background model server on the user's machine and left it running after Oasis quit. Now `AppState.get_llm()`, probed on first `raw=false` request and cached (including the `None`, via a separate `llm_probed` flag). "Once, not per search" is preserved — *startup* was simply the wrong "once". Injection goes through `set_llm()`, because assigning `state.llm` alone would be silently overwritten by a real probe. **Verified live against a running server**: `lsof` shows **no connection to 11434** through startup + a raw search, and an established one immediately after a single `raw=false` request.

**📦 `openai` is now a declared dependency.** `llm/ollama.py` does `import openai` (an OpenAI client pointed at Ollama's `/v1`), but it appeared in neither manifest and resolved only transitively through `instructor` — fine until instructor changes its client stack and a bare import fails inside a frozen bundle nobody can pip-install into. Re-locked and diffed: **a one-line lockfile change**, the constraint itself, no version churn.

**🗑 1.5 GB reclaimed.** `.venv.pre-pixi-SAFE-TO-DELETE/` and a stray `app/Oasis/.mypy_cache/`, both verified orphaned first (git-ignored, untracked, unreferenced, not the live interpreter) and `oasis`/`torch` re-imported afterwards.
