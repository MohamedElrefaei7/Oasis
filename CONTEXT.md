...
Last resynced against the repo: **2026-07-17** (910 tests, all passing, torch-free by default).
...
## Tests — 910, all passing

New 2026-07-17: cancel-binding + hermeticity commit — `test_api_index.py` grew 2 (wrong/stale `job_id` → 409 with the running job's cancel event untouched — the auto-reindex-race guard; finished-job id → 409) and a module-scoped guard asserting `torch` never lands in `sys.modules`; `test_pipeline.py` grew the instrumented cancel→resume idempotency test (`COUNT(*) == COUNT(DISTINCT path) ==` walked indexable files).
...
| `test_pipeline.py` | 41 | `test_walker_edges.py` | 19 | `test_models.py` | 10 |
...
- **Implement the remaining HTTP API endpoints** — skeleton, `/api/search`, `/api/open`, `/api/status`, and now **`/api/index` + SSE events + cancel (2026-07-16)** have landed; the search-during-index regression test is ~~still to build~~ **done** (in `test_api_index.py`). Still to build against the `CLAUDE.md` spec: `/api/reset` (use `state.invalidate()` + rebuild the shared `VectorIndex`). New endpoints go on `protected_router` (populated before `create_app` runs — catch-all shadows late additions).
  - **Design for the NEXT index commit — stale reconciliation + no-vector backfill (recorded precisely now):** the done-vs-cancelled distinction is already wired for this — `_run_job` sets `status = "done"` only when the pipeline returned uncancelled, and the sweep must **gate on `status == "done"`**, never run on `cancelled` or a partial walk ("not seen in the walk" is meaningless for an incomplete walk; sweeping on it deletes everything past the cancel point).
    - Stale-document reconciliation on full reindex. `index_directory` currently only adds/updates; it never deletes. A full reindex of a root should delete stored documents under that root that were NOT seen in the walk (file removed, moved, or newly excluded) — SQLite row + FTS + vectors via `delete_by_doc_id`. Valid ONLY when re-indexing the same root that produced the rows, which is why `indexed_roots` must be persisted. Scope deletion to paths under the indexed root, never global.
    - No-vector backfill. A non-force reindex skips the embed phase for unchanged files, so an index built before vectors (or with vectors partially missing) can report `reindex_recommended` forever and a plain reindex won't fix it. The pipeline should embed any document that has no vector rows even when the file itself is unchanged — so "reindex" actually repairs `semantic_ready` without requiring `--force`.
...
- **The real `~/.oasis` index has no embeddings** (built June 3, pre-vector). Re-index to populate `index.lance`. **Detection is now handled** — `/api/health` reports `documents: 877, schema_version: 0, semantic_ready: false, reindex_recommended: true` against it (verified live 2026-07-16), so the app gets a single server-derived boolean to act on instead of doing version math; what's left is the app-side UX for that prompt. Note: because `is_unchanged` skips untouched files and skipped files never reach the embed phase, populating vectors for this old index needs `--force` — a plain re-run would set `schema_version` but never claim `vectors_built` (by design: markers are only set when the embed phase actually runs).
