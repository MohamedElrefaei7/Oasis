...
**Phase 6 (the Swift app) is underway.** Steps 1–3 are built and verified live: `app/Oasis/` spawns `oasis serve --managed`, reads the handshake, polls `/api/health`, renders warming/ready/failed, and tears the child down on both quit paths (step 1); the main window searches through `/api/search` into a result grid (step 2); and **Index New Folder now works end to end** — `NSOpenPanel` → `POST /api/index` → the SSE progress stream → progress sheet with cancel → terminal summary → document count refreshed, closing the index→searchable loop inside one session (step 3, 2026-07-26). No menu bar, no floating panel, and the other three rail controls are still inert. See "Recently done (2026-07-26)" and "(2026-07-25)".
